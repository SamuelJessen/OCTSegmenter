...
def train_and_validate_cv(root_dir, config, splits, folds, transform, optimizer, criterion, net, device, trial_id):
    ...
    print(f"Training on fold {config['fold']+1} out of {folds}")
    ...
    trainloader = DataLoader(train_dataset, batch_size=config["batch_size"], shuffle=True, num_workers=config["cpus_per_trial"], drop_last=True)
    valloader = DataLoader(val_dataset, batch_size=config["batch_size"], shuffle=False, num_workers=config["cpus_per_trial"], drop_last=True)
